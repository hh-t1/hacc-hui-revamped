...
const EditProfileWidget = (props) => {
  ...
  const buildTheModel = () => {
    const { participant, devChallenges, devSkills, devTools } = props;

    const challenges = devChallenges.map((challenge) => {
      const c = Challenges.findDoc(challenge.challengeID);
      return c.title;
    });
    const skills = devSkills.map((skill) => {
      const s = Skills.findDoc(skill.skillID);
      return s.name;
    });
    const tools = devTools.map((tool) => {
            ...
            >
              <Container>
                <Row>
                  <Col>
                    <h2>
                      Hello {firstname}, this is your first time to login, so
