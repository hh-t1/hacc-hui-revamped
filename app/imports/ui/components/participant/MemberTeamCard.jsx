...
import { Grid, Header, Image, Item, Icon, List } from 'semantic-ui-react';
...
class MemberTeamCard extends React.Component {
  render() {
    ...
    const teamChallenges = _.map(
      TeamChallenges.find({ teamID }).fetch(),
      (tc) => Challenges.findDoc(tc.challengeID).title,
    );
    const teamSkills = _.map(
      TeamSkills.find({ teamID }).fetch(),
      (ts) => Skills.findDoc(ts.skillID).name,
    );
    const teamTools = _.map(
      TeamTools.find({ teamID }).fetch(),
      (tt) => Tools.findDoc(tt.toolID).name,
    );
    return (
      <Item style={{ padding: '0rem 2rem 0rem 2rem' }}>
        <Item.Content>
          ...
          <Item.Meta>
            <Grid columns="equal">
              <Grid.Column>
                GitHub: {this.props.team.gitHubRepo}
                <br />
